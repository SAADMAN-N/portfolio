...
export default function MusicPlayerCard({ tracks, className }) {
  ...
  const togglePlay = () => {
    // No sound plays - UI only (play/pause icon toggle)
    setIsPlaying((p) => !p);
  };
  ...
  return (
    <div
      className={cn(
        "mx-auto w-full max-w-[320px] rounded-2xl border-2 border-border bg-[#171717] p-4",
        className
      )}
    >
      {/* album art + heart */}
