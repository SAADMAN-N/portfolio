...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const calculateWorldPositions = useCallback((): WorldPosition[] => {
    ...
    // Simplified collision detection - only check nearby images for performance
    ...
    for (let i = 0; i < adjustedPositions.length; i++) {
      ...
      // Only check collision with nearby images (within reasonable distance)
      for (let j = 0; j < adjustedPositions.length; j++) {
        ...
        const maxPossibleOverlap = (imageSize + otherSize) / 2 + 50;

        if (distance > maxPossibleOverlap) continue; // Skip distant images

        // Minimum distance to prevent overlap
        const minDistance = (imageSize + otherSize) / 2 + 20;

        if (distance < minDistance && distance > 0) {
          // Simplified scale reduction
          const overlap = minDistance - distance;
          const reductionFactor = Math.max(
            0.5,
            1 - (overlap / minDistance) * 0.3
          );
      ...
      adjustedPositions[i] = {
        ...pos,
        scale: Math.max(0.3, adjustedScale), // Ensure minimum scale
      };
    }
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      // Restart animation when user interacts
    ...
    [velocity, autoRotate, updateMomentum]
  );
  ...
  useEffect(() => {
    const animate = () => {
      // Only animate if there's actual movement or auto-rotation
      const hasMovement =
        Math.abs(velocity.x) > 0.01 ||
        Math.abs(velocity.y) > 0.01 ||
        autoRotate;
  ...
  }, [isMounted, updateMomentum, velocity, autoRotate]);
